...
export class NativeInterpreter extends JSChannel_ {
    ...
    mountedEventQueue: Array<{
        name: string;
        element: number;
        data: any;
        bubbles: boolean;
    }> = [];
    mountedEventFlushScheduled = false;
                ...
                if (target_id !== null) {
                    ...
                    if (
                        ...
                        let requestData = {
                            ...
                            directory:
                                target.getAttribute("webkitdirectory") ===
                                "true",
                        ...
                        this.fetchAgainstHost("__file_dialog", requestData)
                            ...
                            .then((resp) => {
                                const formObjects: SerializedFormObject[] =
                                    resp.values;
                                ...
                                for (let formObject of formObjects) {
                                    if (
                                        ...
                                        const file = new File(
                                            [],
                                            formObject.file.path,
                                            {
                                                type: formObject.file
                                                    .content_type,
                                                lastModified:
                                                    formObject.file
                                                        .last_modified,
    ...
    sendSerializedEvent(body: {
        ...
        if (this.liveview) {
            ...
            return handleVirtualdomEventSync(
                this.eventsPath,
                JSON.stringify(body),
            );
        }
    ...
    queueMountedEvent(body: {
        name: string;
        element: number;
        data: any;
        bubbles: boolean;
    }) {
        this.mountedEventQueue.push(body);
        if (this.mountedEventFlushScheduled) return;

        this.mountedEventFlushScheduled = true;
        queueMicrotask(() => {
...
function handleVirtualdomEventSync(
    ...
    const contents_base64 = btoa(
        String.fromCharCode.apply(null, contents_bytes),
    );
